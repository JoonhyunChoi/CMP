...
public class MovieTable {
	...
	public class movieTable {
		...
		public movieTable(){
			...
			movieSelect[][] 선택정보 = new movieSelect[6][13];
			
			for (int i = 0; i < 선택정보.length; i++) {
				for (int j = 0; j < 선택정보[i].length; j++) {
					선택정보[i][j] = new movieSelect(i+1, 9+j);
					선택정보[i][j].addMouseListener(new MouseAdapter() {
						@Override
						public void mouseClicked(MouseEvent e) {
							/*new SeatReserve(선택정보[i][j].getPlace(),선택정보[i][j].getTime());*/   //정호형이랑 연결할 거
			...
			JFrame mf = new JFrame("==========영화상영 시간표==========");
		       ...
		       for (int i = 0; i < 13; i++) {
		    	   if(i == 0 || i % 2 ==0){
		    		   p2.add(new JButton(선택정보[0][i].getPlace() + " 관" + (9+i) + " : 00"));  
		    	   }else{
		    		   p2.add(new JButton(선택정보[1][i].getPlace() + " 관" + (9+i) + " : 00"));
		    	   }
		/*		p2.add(new JButton(선택정보[0][i].getPlace() + " 관" + 선택정보[0][i].getTime() + " : 00"));
				p2.add(new JButton(선택정보[1][i].getPlace() + " 관" + (선택정보[1][i].getTime()+2) + " : 00"));*/
			}
		      ...
		      /* p2.add(new JButton("1관 | 09:00"));
		       ...
		       for (int i = 0; i < 13; i++) {
		    	   if(i == 0 || i % 2 ==0){
		    		   p3.add(new JButton(선택정보[2][i].getPlace() + " 관" + (9+i) + " : 00"));  
		    	   }else{
		    		   p3.add(new JButton(선택정보[3][i].getPlace() + " 관" + (9+i) + " : 00"));
		    	   }
		       }
		       ...
		       for (int i = 0; i < 13; i++) {
		    	   if(i == 0 || i % 2 ==0){
		    		   p4.add(new JButton(선택정보[4][i].getPlace() + " 관" + (9+i) + " : 00"));  
		    	   }else{
		    		   p4.add(new JButton(선택정보[5][i].getPlace() + " 관" + (9+i) + " : 00"));
		    	   }
		       }
